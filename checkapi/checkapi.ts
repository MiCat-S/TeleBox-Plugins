...
// ── Persistence ──
async function ensurePrivateStore(): Promise<void> { await fs.mkdir(DD,{recursive:true,mode:0o700}); if(process.platform!=="win32")await fs.chmod(DD,0o700); }
async function lk(): Promise<SK[]> { await ensurePrivateStore(); try { if(process.platform!=="win32")await fs.chmod(KF,0o600); return JSON.parse(await fs.readFile(KF,"utf8")); } catch(error:any) { if(error?.code==="ENOENT")return []; throw error; } }
async function sk(k: SK[]): Promise<void> { await ensurePrivateStore(); await fs.writeFile(KF,JSON.stringify(k,null,2),{encoding:"utf8",mode:0o600}); if(process.platform!=="win32")await fs.chmod(KF,0o600); }
...
function redactSensitiveText(value:unknown):string{return String(value??"").replace(/([?&](?:key|api_key|token)=)[^&\s"'<>]+/gi,"$1***").replace(/((?:authorization|x-api-key|api-key)\s*[:=]\s*(?:bearer\s+)?)[^\s,"'}]+/gi,"$1***").replace(/\b(?:sk-(?:ant-|or-v1-)?|gsk_|tgp_|pplx-|r8_|fw_|xai-|co-|hf_|nvapi-)[A-Za-z0-9._-]{6,}\b/gi,"***").replace(/\bAIza[A-Za-z0-9_-]{20,}\b/g,"***");}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function ge(e:unknown):string{const o=e as any; return redactSensitiveText(o?.message||o?.stderr||e||"未知错误");}
